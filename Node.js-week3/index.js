...
server.get("/login/:username/:password",(request, response)=> {
    ...
    const hasedpassword = SHA256(password).toString();
    const sql = `SELECT * FROM user WHERE ?`;
    const values = {username};
    connection.query(sql, values, (error, results) => {
        const dbPassword = results[0].password;
        const Decryptpassword = AES.decrypt(dbPassword.toString(), results[0].salt);
        const sqlPassword = Decryptpassword.toString(enc.Utf8);
        if(error){ showError(error, response);
        }
        if (hasedpassword === sqlPassword) {
            response.json({status: "success",  message: "You are logged in"});
        } else {
            response.json({status: "error",message: "Enter a valid username and password"});
        }
    });
});
